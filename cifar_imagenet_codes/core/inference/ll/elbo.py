...
from core.inference.utils import _x_0_pred, _choice_steps, _report_statistics, _solve_fn_dp_general
import logging
from core.evaluate.score import score_on_dataset


@ torch.no_grad()
def nelbo_naive_ddpm(x_0, betas, small_sigma, clip_denoise, rescale_timesteps,
    ...
    logging.info("nelbo_naive_ddpm with {}, small_sigma={}, clip_denoise={}, sample_steps={}, shift1={}"
                 .format("eps_model" if eps_model is not None else "d_model", small_sigma, clip_denoise, sample_steps, shift1))
    ...
    for s, r in tqdm(list(zip([0] + ns, ns))[::-1]):
        ...
        x_0_pred, eps_pred = _x_0_pred(x_r, r, cum_alphas, rescale_timesteps, eps_model=eps_model, d_model=d_model, shift1=shift1)
        if clip_denoise:
...
@ torch.no_grad()
def nelbo_dp_ddpm(train_nll_terms, test_nll_terms, sample_steps):
    F_train = train_nll_terms['F']
    N = len(F_train) - 1
    ns = _solve_fn_dp_general(F_train, 0, N, sample_steps + 1)
...
@ torch.no_grad()
def get_nelbo_terms(dataset, batch_size, d, betas, small_sigma, rescale_timesteps, eps_model=None, shift1=False):
    ...
    logging.info("get_nelbo_terms with {}, small_sigma={}, shift1={}"
                 .format("eps_model" if eps_model is not None else "d_model", small_sigma, shift1))
    ...
    for r in range(1, N + 1):
        def reconstruction_nll_fn(x_0):
            ...
            x_0_pred, eps_pred = _x_0_pred(x_r, r, cum_alphas, rescale_timesteps, eps_model=eps_model, shift1=shift1)
        ...
        reconstructions[r], F[0, r] = score_on_dataset(dataset, reconstruction_nll_fn, batch_size)
    ...
    for s in range(1, N + 1):  # F is kl when 1 <= s < r <= N
        for r in range(s + 1, N + 1):
            ...
            sigma2 = tilde_beta if small_sigma else skip_beta
            c = 0.5 * d * (tilde_beta / sigma2 + np.log(sigma2 / tilde_beta) - 1.)
...
@ torch.no_grad()
def nelbo_ms_eps_ddpm(x_0, betas, rescale_timesteps, steps_type='linear',
                      eps_model=None, ms_eps=None, sample_steps=None, shift1=False):
    ...
    logging.info("nelbo_ms_eps_ddpm with eps_model, sample_steps={}, steps_type={}, shift1={}"
                 .format(sample_steps, steps_type, shift1))
